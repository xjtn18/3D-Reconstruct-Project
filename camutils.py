...
def decode(imprefix,start,threshold, grab_number):
	"""
	...
	camstr = "C0" if "C0" in imprefix else "C1" # get the correct camera id for the file name
	bg = plt.imread(f"./manny/grab_{grab_number}_u/color_{camstr}_00.png") #background image
	obj = plt.imread(f"./manny/grab_{grab_number}_u/color_{camstr}_01.png") #image witht he object

	#convert bg and obj images to grayscale
	bg = np.mean(bg,axis=2)
	obj = np.mean(obj,axis=2)

	diff = np.abs(bg - obj)
	diff_mask = (diff>threshold).astype(np.int) #create mask where the difference between the 2 images is greater than threshold
	mask = mask.astype(np.int)
	mask = diff_mask & mask #combine the masks using AND
		
	return code,mask



def reconstruct(imprefixL,imprefixR,threshold,camL,camR,grab_number):
	"""
	...
	# pass in the grab numbers as well so we pull the correct images
	CLh,maskLh = decode(imprefixL,0,threshold, grab_number)
	CLv,maskLv = decode(imprefixL,20,threshold, grab_number)
	CRh,maskRh = decode(imprefixR,0,threshold, grab_number)
	CRv,maskRv = decode(imprefixR,20,threshold, grab_number)
	...
	#store each points respective colors
	color_img = plt.imread(f"./manny/grab_{grab_number}_u/color_C0_01.png") # get the right color image
	colors = color_img[pts2R[1,:], pts2R[0,:]].T
